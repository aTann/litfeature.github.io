...
// 添加新的 jQuery 方法，以操作 DOM 元素；
// 使用 jQuery UI 插件工厂创建完善的插件；
// 对外发布完工的插件。
...
// 在任何插件方法内部，关键字 this 引用的都是当前的 jQuery 对象。因而，可以在 this 上面
// 调用任何内置的 jQuery 方法，或者提取它包含的DOM节点并操作该节点。为了确定可以怎样利用
// 对象的上下文，下面我们来编写一个小插件，用以操作匹配元素的类。

// 这个新方法接受两个类名，每次调用更换应用于每个元素的类。尽管 jQuery UI 有一个健壮
// 的 .switchClass() 方法，甚至该方法都支持以动画方式切换类，但为了演示需要，我们还是自
// 己再来写一个吧，参见代码

/*// 首先，测试每个匹配的元素是否已经应用了class1，如果是，则将该类替换成 class2 。然
// 后，再测试 class2 并在必要时替换成 class1。如果两个类都不存在，则什么也不做。
;(function ($) {
...
// 只要有可能通过插件来模仿 jQuery API，就应该使用对象来提高一致性和易用性
...
/*;(function ($) {
	$.fn.shadow = function (opts) {
		...
		// 实用函数$.extend()
		// 可以用接受的 opts 对象参数覆盖 defaults 中的选项，并保持选项对象中未指定的默认项
		// 不变。
...
// 在各种 jQuery API 中经常可以看到另一种参数类型，即回调函数。回调函数可以极大地增加插件的灵活性，但却用不着在创建插件时多编写多少代码。

// 要在方法中使用回调函数，需要接受一个函数对象作为参数，然后在方法中适当的位置上调用该函数。
// 例如，可以扩展前面定义的文本投影方法，让用户能够自定义投影相对于文本的位置
...
// jQueryUI 库的核心包含了一个工厂方法，叫 $.widget()，这个方法能帮我们做很多事情。使用这个方
// 法可以确保我们的代码达到所有 jQuery UI 部件用户认可的 API 标准。
...
// 每次调用 $.widget() 都会通过部件工厂创建一个 jQuery UI 插件。这个函数接受部件的名称
// 和一个包含部件属性的对象作为参数。部件名称必须带命名空间，在这里我们使用 ljq 作为命名
// 空间，使用 tooltip 作为插件名称。这样，在jQuery项目中就可以通过 .tooltip() 调用我们这
// 个插件了。
...
// 为避免$别名与其他库发生冲突，可以使用 jQuery，或者在立即调用的函数表达式（IIFE）
// 中传入$，使其成为一个局部变量。
