...
var eventTargetSet = new Set(eventTargetList);

names = names.filter(n => !eventTargetSet.has(n))

// 移除自建
names = filter('eventTargetSet', 'eventTargetList')
console.log(names.length) // 
...
// "$_", "$0", "$1", "$2", "$3", "$4",
// $(selector, [startNode]) $$(selector, [startNode]) $x(path, [startNode])
// copy(object)
// 'debug', 'undebug',
...
var devToolsList = ['inspect', 'queryObjects', 'getEventListeners', '$', '$$', '$x', 'debug', 'undebug', 'dir', 'dirxml', 'monitor', 'unmonitor', 'monitorEvents', 'unmonitorEvents', 'profile', 'profileEnd', 'table']
var devToolsSet = new Set(devToolsList);

names = names.filter(n => !devToolsSet.has(n))
